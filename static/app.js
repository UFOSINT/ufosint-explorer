...
function populateFilterDropdownsFromDeck() {
    ...
    // v0.15 — mount the Shape multi-select.
...
function _mountShapeMultiSelect(shapes) {
    const wrap = document.getElementById("ms-shape");
    if (!wrap) return;
    const opts = (shapes || [])
        .filter(s => !!s)  // drop the index-0 "(unknown)" placeholder
        .map(s => ({ value: s, text: s }));
    if (_msDropdowns.shape) {
        _msDropdowns.shape.setOptions(opts);
        return;
    }
    _msDropdowns.shape = buildMultiSelect({
        wrapEl: wrap,
        label: "Shape",
        options: opts,
        onChange: () => {
            // Fire applyFilters with no debounce — the factory already
            // dedupes against no-op toggles, and the user expects the
            // map to react instantly on every click.
...
function applyClientFilters() {
    ...
    // v0.15 — Shape is now multi-select. Read from the factory instance
    // if mounted; fall back to the legacy <select> for the boot window
    // before the instance is created. Empty array === no filter.
    let shapeNames = null;
    const shapeMs = _msDropdowns.shape;
    if (shapeMs) {
        const sel = shapeMs.getSelection();
        shapeNames = sel.length > 0 ? sel : null;
    } else {
        const v = document.getElementById("filter-shape")?.value || "";
        shapeNames = v ? [v] : null;
    }

    const filter = {
        sourceName: srcName,
        // v0.15 — `shapeName` (scalar) is superseded by `shapeNames`
        // (array). Both fields are emitted during the transition so
        // deck.js can accept either form; once all filters are
        // multi-select the scalar will be deleted.
...
function loadObservatory() {
    ...
    // v0.12 — eagerly fetch overlay data so crash/nuclear stems
    // appear on the TimeBrush immediately (even before the user
    // clicks an overlay toggle). The fetch is a no-op if already
    // cached from a previous visit.
    if (typeof loadOverlayData === "function") {
        loadOverlayData().then(() => {
            if (state.timeBrush) state.timeBrush._drawAnnotations();
        });
    }
...
// Client-side aggregation from the POINTS bulk buffer (v0.14 schema
// added 8 uint8 NRC columns at offset 40). Sums word counts over the
// current visible set (visibleIdx), so the donut animates in sync
// with TimeBrush scrubbing, bbox pan, and every other cross-filter
// without a network round-trip — matching the other 5 Emotion cards.
//
// The uint8-clamp loses resolution on the ~20 rows of 618k whose
// counts exceed 255 (max observed is ~475 trust). Aggregate-sum
// visual impact is invisible.
//
// Valence totals (positive/negative) were dropped from the footnote
// because nrc_positive/nrc_negative are NULL for all rows in the
// shipped dataset (upstream pipeline never populated them). The
// footnote now shows the visible-sighting count instead.
...
function renderNrcLexicon() {
    ...
    // Bail gracefully on older buffer formats (e.g. CDN served stale
    // pre-v0.14 JS alongside a v0.14 meta, or vice versa).
    if (!P.nrcJoy) return;

    const arrs = [
        P.nrcJoy, P.nrcFear, P.nrcAnger, P.nrcSadness,
        P.nrcSurprise, P.nrcDisgust, P.nrcTrust, P.nrcAnticipation,
    ];
    ...
    if (footnote) {
        footnote.innerHTML = (
            `<span class="nrc-n">${N.toLocaleString()} sightings</span> &middot; ` +
            `<span class="nrc-words">${total.toLocaleString()} emotion words</span>`
        );
    }
