...
def test_app_schema_version_bumped():
    ...
    v0.16.9 bumped it to v0169-1 — not for a layout change (the row is
    still 48 bytes) but to invalidate cached buffers after the NRC
    columns were backfilled in place. Nothing the ETag is built from
    moved, so without the bump every worker and browser would have kept
    serving the zero-filled copy.

    v0.17 bumped it again to v017-1. Source indices in the packed buffer
    are assigned alphabetically, so retiring NUFORC and adding Capella
    shifts the source_idx byte of nearly every row.
    """
    src = _read(APP_PY)
...
def test_app_bytes_per_row_is_28():
    """v0.14 grew the row to 48 bytes (40 v0.11 layout + 8 new
    uint8 NRC word-count columns)."""
    src = _read(APP_PY)
